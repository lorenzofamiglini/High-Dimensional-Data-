...
model1 <- train(y_train ~ PC1_taxa + PC2_taxa + PC3_taxa + PC4_taxa + PC5_taxa + PC6_taxa + age + sex1m2f + cluster, train_data, 
...
train.control <- trainControl(method = "cv", number = 10,savePred=T)
model2 <- train(y_train ~ PC1_nut + PC2_nut + PC3_nut + PC4_nut + PC5_nut + age + sex1m2f + cluster, train_data, 
               method = "lm", trControl = train.control)

model2$resample
Metrics::mape(predict(model2,X_test), y_test)
...
ggplot(final_info)  +
  ...
  scale_color_manual(values=c("blue", "black"), labels = c("CI RMSE", "R-squared adj")) +
labs(title="Intervallo di confidenza CV rispetto al RMSE",
      subtitle="Mettiamo a confronto gli errori ottenuti in cv dai diversi modelli e R-squared adj in holdout",
      y = "RMSE (mean cv)",
...
#Predizioni:
predictions <- predict(mod_lin2, no_out)
