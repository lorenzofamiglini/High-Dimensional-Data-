...
# controllo che non ci siano nutrienti con 97 valori uguali: per la permanova non andrebbero bene
...
fdr_list <- c()
for (i in 2:(ncol(nutrients_table))){
    permanova = adonis(microbiome_distance_unweighted ~ nutrients_table[,i],permutations = 299)
...
map_nutrients[,ncol(map_nutrients)]
...
#final_nutrients = tabella_nutrienti[,selected_fdr_indexes]
#rownames(final_nutrients) = tabella_nutrienti$id
``` 
...
microbiome_nutrients_data = merge(final_nutrients,genera_names,by=0,sort=FALSE)
