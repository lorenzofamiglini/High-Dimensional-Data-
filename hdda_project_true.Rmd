...
physeq_genera_all <- aggregate_taxa(physeq, level = "taxonomy6")
#physeq_genera <- subset_taxa(physeq_genera_all, taxonomy6!="g__")
...
# transform all columns to grams
...
#taxa_table_6_num = as.data.frame(lapply(taxa_table_6,function(x) as.numeric(as.character(x))))
#taxa_table_6_ab = as.data.frame(t(apply(taxa_table_6_num,1, function(x){100*x/sum(x)})))
#rownames(taxa_table_6_ab) = rownames(taxa_table_6)

#taxa_table_6_ab$k__Bacteria.p__Bacteroidetes.c__Bacteroidia.o__Bacteroidales.f__Bacteroidaceae.g__Bacteroides
...
```{r}
# Creating Heatmap with correlation between selected nutrients and 76 selected taxa.
...
# merge columns with same name (= equal genera) to plot in heatmap
...
#ggplot(data = correlation_data, aes(x=Taxa, y=Nutrient, fill=correlation)) + 
#  geom_tile() +
...
heatmaply(nutrients_microbiome_correlations,colors = hcl.colors(100,palette="RdBu"), k_row = 4, k_col = 4,
          main ="Correlations between Genera Taxa and Nutrients",fontsize_row = 8,column_text_angle = 90) %>% layout(height=1300,width=900) 

# ATTENZIONE: i risultati della nostra heatmap sono diversi rispetto a quelli del paper: bacteroides poco correlato con i nutrienti da noi, mentre nel paper molto correlato.
...
#prevotella_cl1 = (cluster1_abundances$k__Bacteria_p__Bacteroidetes_c__Bacteroidia_o__Bacteroidales_f__Prevotellaceae_g__Prevotella + cluster1_abundances$k__Bacteria_p__Bacteroidetes_c__Bacteroidia_o__Bacteroidales_f__Pararevotellaceae_g__Prevotella)  # ATTENZIONE: esiste anche [Prevotella]! va fatta pulizia dopo la funzione aggregate_taxa
#prevotella_cl2 = (cluster2_abundances$k__Bacteria_p__Bacteroidetes_c__Bacteroidia_o__Bacteroidales_f__Prevotellaceae_g__Prevotella + cluster2_abundances$k__Bacteria_p__Bacteroidetes_c__Bacteroidia_o__Bacteroidales_f__Pararevotellaceae_g__Prevotella)
#bacteroides_cl1 = cluster1_abundances$g__Bacteroides
#bacteroides_cl2 = cluster2_abundances$g__Bacteroides

# preparing data for bacteroides boxplot
#bacteroides_ab = c(bacteroides_cl1,bacteroides_cl2)
#bacteroides_cluster = c(rep(1,length(bacteroides_cl1)),rep(2,length(bacteroides_cl2)))
#bacteroides = as.data.frame(cbind(bacteroides_ab,bacteroides_cluster))
#colnames(bacteroides) = c("Bacteroides abundance","cluster")
...
```{r}
#all_varss = read.csv("all_vars_df.csv", sep = ",")
#genera_all_vars =  read.csv("datasets/genera_all_vars_df.csv", sep = ",")
```
...
microbiome_nutrients_data = merge(genera,final_nutrients,by=0)
rownames(microbiome_nutrients_data) = microbiome_nutrients_data[,1]
microbiome_nutrients_data[,c("Row.names")] <- list(NULL)
microbiome_nutrients_data[,c("id")] <- list(NULL)

nutrients_microbiome_correlations = cor(microbiome_nutrients_data,method="spearman")[1:ncol(genera),(ncol(genera)+1):ncol(microbiome_nutrients_data)]


correlation_data <- melt(nutrients_microbiome_correlations)
names(correlation_data) = c("Taxa","Nutrient","correlation")
correlation_data$Taxa = as.character(correlation_data$Taxa)
correlation_data$Taxa = unlist(lapply(correlation_data$Taxa, function(x) strsplit(x,"g__")[[1]][2]))
...
>>>>>>> 93b520d60a814df3d36e498c5f47354c889f303a
>>>>>>> 9a248d2a925185301834dd8c1775dc2f1e0b5369
