...
```{r}
#Prendiamo tutti i taxa senza aggregazione 
...
random_tree = rtree(119, rooted=TRUE, tip.label=taxa_names(physeq2))
plot(random_tree,show.tip.label = F)
phy_tax = merge_phyloseq(physeq2, random_tree)
...
nut_true <- nutriens2[match(get_id, nutriens2$id),]
```
...
for (i in 2:ncol(nut_true)){
    perm <- RVAideMemoire::pairwise.perm.manova(distanza,nut_true[,i],nperm=999) 
    app <- as.vector(perm$p.value)
    p_val <- min(app[!is.na(app)]) 
    lista = append(lista,p_val)
}
``` 
...
df_lista$column <- c(1:152)

df_li <- subset(df_lista, lista <= 0.25)


nut_true2 <- nutriens[,1:ncol(nutriens)]
final_nutriens <- nut_true2[,c(1,df_li$column)]
...
final_nutriens$id <- gsub("1011.C.", "", final_nutriens$id)

final_nutriens$id <- gsub(".01.P\\d|.01.S\\d", "", final_nutriens$id)

final_nutriens

bmi_df <- readxl::read_excel("datasets/bmi calculation with bmicategories.xlsx")
colnames(bmi_df)[1] <- "id"

bmi_nutriens <- merge(final_nutriens, bmi_df, by = "id", sort = F)
```
