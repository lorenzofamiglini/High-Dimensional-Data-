...
```{r}
# Compute Jensen Shannone divergence matrix on gut microbiomes based on 119 taxa obtained.
...
```{r}
# Selected clustering method with 2 clusters.
selected_clustering <- cluster::pam(dist_jsd,2,diss = T, metric = "euclidean") #get cluster 
cluster_assignment = as.vector(selected_clustering$clustering)

# Perform multidimensional scaling on JSD matrix
multidimensional_scaling <- cmdscale(dist_jsd,eig=TRUE, k=2) # k is the number of dim

# cumulative percentage of variance explained by first 2 dimensions
cumulative_variance_explained_mds = multidimensional_scaling$GOF 

# Plot individuals in two cluster on 2 dimension from MDS output
x <- multidimensional_scaling$points[,1]
y <- multidimensional_scaling$points[,2]
plot (x,y,col=cluster_assignment, ylim = c(-0.5,0.5),pch=19)

# Individuate medoids
selected_clustering$medoids
medoid_cl1 = multidimensional_scaling$points[rownames(multidimensional_scaling$points) == selected_clustering$medoids[1],]
medoid_cl2 = multidimensional_scaling$points[rownames(multidimensional_scaling$points) == selected_clustering$medoids[2],]

# ATTENZIONE: plottare meglio i dati: mettere i medoidi e ellissi..
```

```{r}
# differenze tra i cluster

patients_clusters = as.data.frame(selected_clustering$clustering)
names(patients_clusters) = "cluster"

table_taxa_clusters = merge(otu_table_genera, patients_clusters, by = 0)
...
prevotella_cl1 = cluster1_abundances$g__Prevotella # ATTENZIONE: esiste anche [Prevotella]! va fatta pulizia dopo la funzione aggregate_taxa
prevotella_cl2 = cluster2_abundances$g__Prevotella
